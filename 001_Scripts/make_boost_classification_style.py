...
# =========================
target_styles = [
    ['Impressionism (1880)', 'Impressionism'],
    ['Realism (1880)', 'Realism'],
    ['Romanticism (1820)', 'Romanticism'],
    ['Expressionism (1910)', 'Expressionism'],
    ['Baroque (1630)', 'Baroque'],
    ['Post-Impressionism (1880)', 'Post-Impressionism'],
    ['Art Nouveau (Modern) (1910)', 'Art Nouveau (Modern)'],
    ['Symbolism (1900)', 'Symbolism'],
    ['Northern Renaissance (1510)', 'Northern Renaissance'],
    ['Rococo (1780)', 'Rococo'],
]
alias = {long: short for long, short in target_styles}
valid_labels = set(alias.keys()) | set(alias.values())

# Style을 두 집합(any)에 포함되는 것만 필터 및 정규화
temp = file_info_latents[file_info_latents['Style'].isin(valid_labels)].copy()
temp['style_norm'] = temp['Style'].map(lambda s: alias.get(s, s))
...
enc = LabelEncoder()
y_all_style = temp['style_norm'].tolist()
y_all_bin = enc.fit_transform(y_all_style)            # 정수 라벨
...
predict_wide = {}     # 시드별 테스트 인덱스/정답/예측 (style 라벨)
...
df_predicts.to_csv(os.path.join(out_dir, "predicts_style.csv"), index=False)
df_trains.to_csv(os.path.join(out_dir, "trains_style.csv"), index=False)
df_metrics.to_csv(os.path.join(out_dir, "metrics_style.csv"), index=False)

print("\n=== Saved ===")
print(f"- {os.path.join(out_dir, 'predicts_style.csv')}")
print(f"- {os.path.join(out_dir, 'trains_style.csv')}")
print(f"- {os.path.join(out_dir, 'metrics_style.csv')}")
print(f"- {os.path.join(out_dir, 'seeds_style.pkl')}")