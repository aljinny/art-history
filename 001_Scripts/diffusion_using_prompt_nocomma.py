...
with open(base_path+'datas/words/tokens.pkl', 'rb') as file:
...
for i in range(4) :
    ...
    for idx,res in enumerate(paintings[year]):
        ...
        result = run_diffusion_model_changing_denoising_steps(init_latent = result, prompt = prompt, n = 1, num = 1, batch = batch, model = model, device = device, step = 5)
        result = result[0].reshape(1,4,64,64)
        temp5.append(result)
            
        result = torch.tensor(res)
        result = result.to(device)   
        result = run_diffusion_model_changing_denoising_steps(init_latent = result, prompt = prompt, n = 1, num = 1, batch = batch, model = model, device = device, step = 10)
        result = result[0].reshape(1,4,64,64)
        temp10.append(result)

        result = torch.tensor(res)
        result = result.to(device)   
        result = run_diffusion_model_changing_denoising_steps(init_latent = result, prompt = prompt, n = 1, num = 1, batch = batch, model = model, device = device, step = 20)
        result = result[0].reshape(1,4,64,64)
        temp20.append(result)

        result = torch.tensor(res)
        result = result.to(device)   
        result = run_diffusion_model_changing_denoising_steps(init_latent = result, prompt = prompt, n = 1, num = 1, batch = batch, model = model, device = device, step = 30)
        result = result[0].reshape(1,4,64,64)
...
with open(base_path+'datas/vectors/diffusion/no_comma/avec/step5/using_prompt.pkl', 'wb') as file:
    pickle.dump(result_5, file)

with open(base_path+'datas/vectors/diffusion/no_comma/avec/step10/using_prompt.pkl', 'wb') as file:
    pickle.dump(result_10, file)

with open(base_path+'datas/vectors/diffusion/no_comma/avec/step20/using_prompt.pkl', 'wb') as file:
    pickle.dump(result_20, file)

with open(base_path+'datas/vectors/diffusion/no_comma/avec/step30/using_prompt.pkl', 'wb') as file:
...
with open(base_path+'datas/vectors/diffusion/no_comma/img/step5/using_prompt.pkl', 'wb') as file:
    pickle.dump(result_5, file)

with open(base_path+'datas/vectors/diffusion/no_comma/img/step10/using_prompt.pkl', 'wb') as file:
    pickle.dump(result_10, file)

with open(base_path+'datas/vectors/diffusion/no_comma/img/step20/using_prompt.pkl', 'wb') as file:
    pickle.dump(result_20, file)

with open(base_path+'datas/vectors/diffusion/no_comma/img/step30/using_prompt.pkl', 'wb') as file:
    pickle.dump(result_30, file)